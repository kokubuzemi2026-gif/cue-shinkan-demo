...
- 応答文の切り詰め（先頭2000字）の副作用（2000字目が宛先アドレスの `@` の手前に落ちると
  ...
  （`responseCode` が無ければ `unknown`）へ落ちる。壊れるアライメントは
  **トークンの長さ − 1 通り**（独立レビューの実測: `550` で2通り、`550-5.4.5` で8通り、
  `ECONNREFUSED` で11通り）。Gmailは上限の理由を先頭行に置き、実応答は最長でも
  400字程度のため実害はないが、上限値を変える場合はここを再確認する
