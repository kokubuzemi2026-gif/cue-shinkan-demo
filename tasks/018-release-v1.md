...
- [x] Task 010・011・013〜017が`develop`へmerge済み（008〜017 + 019 = `05e2701`、+ #23 = `72f3cc8`）
...
3. 接続先だけ設定してH3（SMTP）が未設定だと、新規プロジェクトでは新入生にOTPが1通も届かない
...
#### 再レビュー（3周目・4周目）の結論と対応

**3周目: 承認不可（Blocker 3件）→ 4周目で3件とも閉鎖を確認。**
**4周目: 承認不可（Blocker 2件・いずれも §7.2 の新設箇所）。**

| 周 | 指摘 | 内容 | 対応 |
|---|---|---|---|
| 3 | **B2** | 検証ステップの失敗メッセージが「**Variables** で設定してください」と、**D054が禁じた置き場所へ誘導**していた。同じファイルの別コメントは「variableへ戻さないこと」で自己矛盾。**このメッセージが出るのはH7を初めてやる未設定時**＝D054が最も効いてほしい瞬間で、指示どおりVariableを作っても `secrets.*` は空のまま同じエラーが再発し、「ワークフローが間違っている」と判断して secrets を vars へ戻す動線が開いていた | 誘導先を `Secrets`（**Variables ではありません**）へ。本文の "variable" も secret へ統一 |
| 3 | **B3** | **正本が今も variable を指示していた**。`docs/runbook_operations.md` §2 は環境変数の置き場所の正本でありながら「secret ではなく variable でよい」を断定形で残し、`launch_plan` §7 は H7 の3行下で secret と variable を併記、README も variables のまま | 3ファイル＋secret rotation表を secret へそろえた |
| 3 | **B1** | E3をP1へ格上げした後、`tasks/018` だけ **P1:6** のまま（他2文書は7）。しかも同ファイルの対応表に「両方をP1:7へ直した」という**事実でない記録**があった | P1:7 + E3 を列挙。付与当時の数値は履歴として明示 |
| 4 | **B-1** | §7.2 が「公開までに必要なものだけ」と宣言しながら **H3（SMTP設定）を落としていた**。H6で「新規作成」を選ぶとH3は未設定に戻り、**2026-06-03以降の新規Freeプロジェクトは標準プロバイダのままだとテンプレート変更不可・組織メンバー宛にしか配信されない**。字義どおり実行すると**新入生が誰もログインできないv1.0を公開できてしまう**。§7.1 E6（送信元ドメインの判断）も §7.2 から素通りしていた | 段階1へ **H3** を新設（新規作成時は必須・成功判定は「組織メンバー以外の大学メールに6桁コードが届く」）。H6へ E6 の判断を追加 |
| 4 | **B-2** | §7.2 H8 の「なぜ」が **「OTPのリンク先が壊れる」＝事実誤り**。本アプリは `emailRedirectTo` を渡さず `verifyOtp` の6桁コード方式で `detectSessionInUrl: false`、**リンクを使わない**。さらに「Redirect URLs に入れる」という指示が `runbook_supabase_hosted.md` §3-3「Redirect URLsは追加しない」と**正面から矛盾**。成功判定「OTP往復が通る」も**Site URLの有無で結果が変わらず、H8の充足を判定できない**（H8はmergeゲートなのに） | 「なぜ」を実態へ（テンプレートを既定へ戻した場合に `{{ .SiteURL }}` がlocalhostを指す）。**Site URLのみ**へ変更、Redirect URLsは追加しないと明記。成功判定を観測可能な「URL Configuration の Site URL が公開URLになっている」へ |

**あわせて直した Non-blocker**: 現在地の `develop` SHA（`05e2701`→`72f3cc8`）/ READMEのdecision範囲（D053→D054）/ H6の成功判定の画面名を `runbook_supabase_hosted.md` §5 に合わせて「Project Settings → **API**」へ / H1の手順順序を `runbook_operations.md` §3 の実際の並びへ / workflowコメントの用語。

**持ち越し**: `tasks/017` の「Actions variables」表記（完了済みタスクの記録）。§8 進捗ログの表が行間の空行でGFM上分断される（develop由来の既存問題）。
