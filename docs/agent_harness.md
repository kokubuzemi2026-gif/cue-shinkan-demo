...
このリポジトリでAIエージェント（主にClaude Code）が自走するための実行規約です。`CLAUDE.md` と `AGENTS.md` を、実行手順のレベルまで具体化したものです（D035）。
...
エージェントが応答を終えるたび（Stopイベント）に、`app/` に変更がある場合だけ `npm run lint` → `npm run test -- --run` → `npm run build` を実行します。失敗したらStopをブロックし、失敗コマンドと出力の末尾をエージェントへ返します。
...
- `stop_hook_active` が true のとき（直前のブロックによる継続中）は即座に終了し、
  無限ループを防ぎます。そのため**1回の継続につきブロックは1回まで**で、
  ブロック後に修正しないまま終了しても2回目は素通りします
  （利用者の新しい指示でターンが変われば、再びブロックできます）。
  取りこぼしの担保はCIが行います。
...
- ローカル品質ゲートは、1回の継続につき1回しかブロックできません（無限ループ防止のため）。
  ブロック後に修正しないまま終了することを止められません。
