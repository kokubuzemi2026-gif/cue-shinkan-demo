...
- **入口意図（D056）も同じ扱い**: `EntryIntent`はログイン後の初期表示を決めるだけのUI意図で、role・membership・permissionではなく、**認可根拠にしない**。React stateのみに保持し（永続化しない）、初期コンテキストは`initialContextForIntent`が**サーバーの返した所属の中から**選ぶ。入口を選んだだけでは権限・団体・membershipは作られない（作るのは従来どおり`register_student`相当・`create_organization`・`accept_invitation`の明示操作だけ）。
